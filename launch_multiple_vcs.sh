...
while [ $numClients -gt 0 ]; do
  ...
  ./$buildDir/ioquake3.$arch +set r_fullscreen 0 +set sv_pure 0 +set vm_ui 0 +set vm_game 0 +set vm_cgame 0 +set fs_basepath $basepath +set virtualClient 2 +set virtualClientSkill $skill +set virtualClientBot $randBot +set virtualClientName $clientNamePrefix$loopCounter +connect $server &
done
